  ...
  async.waterfall(
    [
      ...
      function (ticketType, group, savedUser, next) {
        settingSchema
          .getSettingByName('ticket:status:default')
    ...
    ticketModel.getTicketById(oId, function (err, ticket) {
      ...
      async.parallel(
        [
          ...
          function (cb) {
            if (!_.isUndefined(reqTicket.firstCategory)) {
              ticket.firstCategory = sanitizeHtml(reqTicket.firstCategory).trim();
            }
            return cb();
          },
          function (cb) {
            if (!_.isUndefined(reqTicket.secondCategory)) {
              ticket.secondCategory = sanitizeHtml(reqTicket.secondCategory).trim();
            }
            return cb();
          },
          function (cb) {
            if (!_.isUndefined(reqTicket.area)) {
              ticket.area = sanitizeHtml(reqTicket.area).trim();
            }
            return cb();
          },
          function (cb) {
            if (!_.isUndefined(reqTicket.areaManager)) {
              ticket.areaManager = sanitizeHtml(reqTicket.areaManager).trim();
            }
            return cb();
          },
          function (cb) {
            if (!_.isUndefined(reqTicket.form)) {
              ticket.form = sanitizeHtml(reqTicket.form).trim();
            }
            return cb();
          },
          function (cb) {
            if (!_.isUndefined(reqTicket.preferredChannel)) {
              ticket.preferredChannel = sanitizeHtml(reqTicket.preferredChannel).trim();
            }
            return cb();
          },
          function (cb) {
            if (!_.isUndefined(reqTicket.summary)) {
              ticket.summary = sanitizeHtml(reqTicket.summary).trim();
            }
            return cb();
          },
          function (cb) {
            if (!_.isUndefined(reqTicket.clientId)) {
              ticket.clientId = sanitizeHtml(reqTicket.clientId).trim();
            }
            return cb();
          },
        ],
